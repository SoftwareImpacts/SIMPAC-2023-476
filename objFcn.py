...
class Node:
    def __init__(self, id:  int, x: float, objFcn: float, demand: float, ready_time: float, due_time: float, service_time: float):
        super()
        self.id = id
...
class objUpdate:
    ...
    def create_from_file(self, file_path):
        ...
        for i in range(node_num):
            ...
            for j in range(i+1, node_num):
                node_b = nodes[j]
                node_dist_mat[i][j] = objUpdate.calculate_dist(node_a, node_b)
    ...
    def calculate_dist(node_a, node_b):
        return np.linalg.norm((node_a.x - node_b.x, node_a.objFcn - node_b.objFcn))
